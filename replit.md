...
- **Five tabs:** The Locals (vendors + establishments with embedded mini-map and All/Vendors/Businesses segmented filter), Browse (vendor search/list), Events (upcoming events from `/api/events` with embedded mini-map), Final Sale (surplus products from `/api/feed/local-now → surplus`), Saved (AsyncStorage-backed favorites, with mini-map of saved vendor locations).
- **Auth + onboarding:** `context/AuthContext.tsx` mirrors the web `UserContext` — session token stored in AsyncStorage under `ol_session`, hydrated on launch via `GET /api/auth/me`. On first launch (no session, `ol_seen_onboarding_v1` not set), `components/OnboardingModal.tsx` shows a role chooser (shopper vs vendor) that routes into `app/(auth)/signup.tsx`, a 5-step modal (role → username+ZIP → avatar style → email → 6-digit code) hitting `/api/auth/check-username`, `/api/auth/signup/{start,resend,verify}`. ZIP step has a "Use my location" button using `expo-location` foreground permission + `reverseGeocodeAsync`. Vendors are auto-routed to `app/(auth)/tiers.tsx` after signup to view Basic/Standard/Premium plans; checkout itself opens the web `/billing` page via `Linking.openURL` (Stripe checkout stays on web for now). Logged-in users see their `Avatar` in The Locals header (long-press to log out); logged-out users see a "Sign in" pill that opens signup.
- **Avatar:** `components/Avatar.tsx` renders the DiceBear PNG (via `expo-image`) plus absolutely-positioned wardrobe overlays from `lib/unlockCatalog.ts`. Overlays load from the published web app's `/wardrobe/` directory (`https://${EXPO_PUBLIC_DOMAIN}/wardrobe/...`) so we don't have to duplicate the PNGs into the mobile bundle. Same equippable items and zones as the web catalog.
