...
export default function VisitRequestsPanel({ vendorId }: { vendorId: number }) {
  ...
  const reload = async () => {
    ...
    try {
      ...
      toast({ variant: "destructive", title: t("visitRequests.toastCouldntLoad"), description: (e as Error).message });
    } finally {
      setLoading(false);
    }
  };
  ...
  const decide = async (id: number, action: "approve" | "reject") => {
    ...
    try {
      ...
      toast({
        title: action === "approve" ? t("visitRequests.toastApproved") : t("visitRequests.toastRejected"),
        description: action === "approve"
          ? (data.newlyUnlockedForShopper?.length
              ? t("visitRequests.toastVisitCreditedWithRewards", { count: data.newlyUnlockedForShopper.length })
              : t("visitRequests.toastVisitCredited"))
          : t("visitRequests.toastRequestDismissed"),
      });
    } catch (e) {
      toast({ variant: "destructive", title: t("visitRequests.toastCouldntUpdate"), description: (e as Error).message });
    } finally {
      ...
      <CardContent className="p-6">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-2">
            ...
            <h2 className="font-serif text-xl font-bold">{t("visitRequests.panelTitle")}</h2>
          ...
          <p className="text-xs text-muted-foreground">{t("visitRequests.panelSubtitle")}</p>
        </div>
        ...
        {loading ? (
              ...
              return (
                <li key={p.id} className="py-3 flex items-center gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-muted-foreground">{t("visitRequests.requestedAt", { date: new Date(p.requestedAt).toLocaleString() })}</p>
                  </div>
                  <Button size="sm" variant="outline" disabled={busy} onClick={() => decide(p.id, "reject")} className="gap-1">
                    <X className="w-4 h-4" /> {t("visitRequests.rejectButton")}
                  </Button>
                  <Button size="sm" disabled={busy} onClick={() => decide(p.id, "approve")} className="gap-1">
                    {busy ? <Loader2 className="w-4 h-4 animate-spin" /> : <Check className="w-4 h-4" />} {t("visitRequests.approveButton")}
                  </Button>
                </li>
              );
            })}
          ...
          <p className="text-sm text-muted-foreground py-4">{t("visitRequests.emptyState")}</p>
        )}
      </CardContent>
    </Card>
  );
