...
const CONTEXT_META: Record<string, { labelKey: string; icon: typeof Search; color: string }> = {
  products: { labelKey: "searchInsights.contextProducts", icon: ShoppingBag, color: "bg-green-100 text-green-700" },
  vendors:  { labelKey: "searchInsights.contextVendors",  icon: Store,       color: "bg-blue-100 text-blue-700" },
  listings: { labelKey: "searchInsights.contextListings", icon: HandHelping, color: "bg-violet-100 text-violet-700" },
  events:   { labelKey: "searchInsights.contextEvents",   icon: CalendarDays,color: "bg-amber-100 text-amber-700" },
  surplus:  { labelKey: "searchInsights.contextSurplus",  icon: Percent,     color: "bg-orange-100 text-orange-700" },
};
...
function QueryBar({ query, count, max, zeroResultCount, contexts, rank, t }: {
          ...
          <div className="flex items-center gap-2 flex-shrink-0">
            {isGap && (
              <span className="inline-flex items-center gap-1 text-xs font-bold bg-red-100 text-red-700 px-2 py-0.5 rounded-full">
                <AlertCircle className="w-2.5 h-2.5" /> {t("searchInsights.noResults", { count: zeroResultCount })}
              </span>
            )}
      ...
      <div className="bg-muted border-b border-border">
        <div className="container max-w-6xl mx-auto px-4 py-10">
          <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4">
            <div>
              <p className="text-xs font-semibold uppercase tracking-widest text-primary mb-2">{t("searchInsights.adminAnalytics")}</p>
              <h1 className="text-4xl font-serif font-bold text-foreground">{t("searchInsights.pageTitle")}</h1>
              <p className="text-muted-foreground mt-1.5 font-sans max-w-xl">
                {t("searchInsights.pageDescription")}
              </p>
            </div>
            <button
              onClick={() => refetch()}
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mt-8">
            {isLoading ? (
              ...
              <>
                <Stat icon={Search} label={t("searchInsights.statTotalSearches")} value={data?.totalSearches ?? 0} />
                <Stat icon={BarChart3} label={t("searchInsights.statUniqueTerms")} value={data?.uniqueTerms ?? 0} sub={t("searchInsights.statUniqueTermsSub")} />
                <Stat
                  icon={AlertCircle}
                  label={t("searchInsights.statZeroResult")}
                  value={data?.zeroResultCount ?? 0}
                  sub={t("searchInsights.statZeroResultSub")}
                  accent
                />
                <Stat icon={TrendingUp} label={t("searchInsights.statSearchedContexts")} value={data?.byContext?.length ?? 0} sub={t("searchInsights.statSearchedContextsSub")} />
              </>
            )}
          </div>
        </div>
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div>
              <div className="flex items-center justify-between gap-4 mb-1">
                <div className="flex items-center gap-2">
                  {view === "opportunities"
                    ? <><Target className="w-5 h-5 text-red-500" /><h2 className="text-xl font-serif font-bold">{t("searchInsights.vendorOpportunities")}</h2></>
                    : <><TrendingUp className="w-5 h-5 text-primary" /><h2 className="text-xl font-serif font-bold">{t("searchInsights.allTopSearches")}</h2></>
                  }
                </div>
                <div className="flex gap-1 bg-muted border border-border rounded-lg p-1">
                  {(["opportunities", "all"] as const).map((v) => (
                    ...
                    >
                      {v === "opportunities" ? `🎯 ${t("searchInsights.tabOpportunities")}` : `📊 ${t("searchInsights.tabAllSearches")}`}
                    </button>
                  ))}
                </div>
              </div>
              <p className="text-sm text-muted-foreground">
                {view === "opportunities"
                  ? t("searchInsights.opportunitiesDesc")
                  : t("searchInsights.allSearchesDesc")
                }
              </p>
            </div>
            ...
            {isLoading ? (
              ...
              <div className="text-center py-16 text-muted-foreground border border-dashed border-border rounded-xl">
                ...
                <p className="font-medium">{t("searchInsights.emptyTitle")}</p>
                <p className="text-sm mt-1">{t("searchInsights.emptyDesc")}</p>
              </div>
            ) : (
          ...
          <div className="space-y-6">
            ...
            <div className="border border-border rounded-xl p-5">
              <h3 className="font-serif font-bold text-base mb-4 flex items-center gap-2">
                <BarChart3 className="w-4 h-4 text-primary" /> {t("searchInsights.searchesByPage")}
              </h3>
              {isLoading ? (
                <div className="space-y-2">{[...Array(5)].map((_, i) => <Skeleton key={i} className="h-8" />)}</div>
              ) : (data?.byContext ?? []).length === 0 ? (
                <p className="text-sm text-muted-foreground">{t("searchInsights.noDataYet")}</p>
              ) : (
                    ...
                    return (
                      <div key={c.context} className="flex items-center justify-between">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-sm">{meta ? t(meta.labelKey) : c.context}</span>
                        </div>
            ...
            <div className="border border-amber-200 bg-amber-50 rounded-xl p-5">
              <div className="flex items-start gap-2 mb-2">
                ...
                <h3 className="font-bold text-sm text-amber-900">{t("searchInsights.tipTitle")}</h3>
              </div>
              <ul className="text-xs text-amber-800 space-y-1.5 leading-relaxed">
                <li>• <strong>{t("searchInsights.tipZeroResultBold")}</strong> {t("searchInsights.tipZeroResultText")}</li>
                <li>• <strong>{t("searchInsights.tipHighCountBold")}</strong> {t("searchInsights.tipHighCountText")}</li>
                <li>• <strong>{t("searchInsights.tipVendorSearchBold")}</strong> {t("searchInsights.tipVendorSearchText")}</li>
                <li>{t("searchInsights.tipShareOpportunities")}</li>
              </ul>
            </div>
            ...
            <div className="border border-border rounded-xl p-5">
              <h3 className="font-serif font-bold text-base mb-4 flex items-center gap-2">
                <Clock className="w-4 h-4 text-primary" /> {t("searchInsights.recentSearches")}
              </h3>
              {isLoading ? (
                    ...
                    return (
                      <div key={i} className="flex items-center justify-between gap-2">
                        ...
                        <span className={`text-[10px] font-semibold px-1.5 py-0.5 rounded-full flex-shrink-0 ${meta?.color ?? "bg-muted text-muted-foreground"}`}>
                          {meta ? t(meta.labelKey) : r.context}
                        </span>
                      </div>
                    );
                  })}
